...
class DecoderLayer(torch.nn.Module):
    ...
    def forward(self, x, encode_out, mask=None, lookup_mask=None, w_out=False, **kwargs):
        # print(x.shape,lookup_mask.shape)
...
class Decoder(torch.nn.Module):
    def __init__(self, num_layers, d_model, input_vocab_size, rate=0.1, max_len=None):
        ...
        self.enc_layers = torch.nn.ModuleList(
            [DecoderLayer(d_model, rate, h=self.d_model // 256, additional=False, max_seq=max_len)
             for _ in range(num_layers)])
        self.dropout = torch.nn.Dropout(rate)

    def forward(self, x, mask=None,encode_out=None,lookup_mask=None):
        weights = []
        # adding embedding and position encoding.
        # print(x)
        x = self.embedding(x.to(torch.long))  # (batch_size, input_seq_len, d_model)
        ...
        x = self.dropout(x)
        for i in range(self.num_layers):
            x,_,w = self.enc_layers[i](x,encode_out, mask, lookup_mask,w_out=True)
            weights.append(w)
        return x, weights # (batch_size, input_seq_len, d_model)

...
def parse_manifest(manifest):
    ...
    return audio_files, codes, pitchs


def get_dataset_filelist(h):
    training_files, training_codes,training_f0s = parse_manifest(h.input_training_file)
    validation_files, validation_codes,validation_f0s = parse_manifest(h.input_validation_file)

    return (training_files, training_codes,training_f0s), (validation_files, validation_codes,validation_f0s)
...
def parse_speaker(path, method):
    ...
    if method == 'parent_name':
        ...
        #spkr_name = path.name.split('_')[0]
        spkr_name = path.parent.name
        #if spkr_name[:3]=="Ses":
        #    spkr_name = path.name.split('_')[0][:5] + path.name.split('_')[-1][0]
        #print("spkr name", spkr_name)
        return spkr_name
    elif method == 'single':
        return 'A'
...
class CodeDataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, code_hop_size, n_fft, num_mels,
        ...
        self.audio_files, self.codes,self.pitchs = training_files
        random.seed(1234)
        ...
        if self.multispkr:
            ...
            #spkrs = ['en_chem', 'en_chess', 'en_dl', 'en_eh', 'en_hs']
            spkrs.sort()

            self.id_to_spkr = spkrs
            self.spkr_to_id = {k: v for v, k in enumerate(self.id_to_spkr)}
            print("self.spkr_to_id in resynth",self.spkr_to_id)
    ...
    def __getitem__(self, index):
        ...
        if self.vqvae:
            ...
            #print(audio.shape[0], self.code_hop_size, self.codes[index].shape[0])
            #print(audio.shape[0] // self.code_hop_size, self.codes[index].shape[0])
            code_length = min(audio.shape[0] // self.code_hop_size, self.codes[index].shape[0])
            code = self.codes[index][:code_length]
            #pitch = self.pitchs[index][:code_length]
            #print("codes length",self.codes[index].shape,code.shape)
        ...
        if self.vqvae:
            ...
            #print("code after interval",code.shape)
        ...
        if self.f0:
            ...
            #feats['pitch'] = np.array(pitch)

        if self.multispkr:
            feats['spkr'] = self._get_spkr(index)

        if False: # self.noise_label:
            feats['noise_label'] = self._get_noiselabel(index)

        if self.f0_normalize and self.f0_stats is not None:
        ...
        #print("@@@dataset : feats code",feats['code'].shape)
        #print('***********',filename,feats['code'].shape,  audio.squeeze(0).shape, mel_loss.squeeze().shape)
    ...
    def _get_spkr(self, idx):
        ...
        #spkr_name = 'en_chem' #parse_speaker(self.audio_files[idx], self.multispkr)
        spkr_id = torch.LongTensor([self.spkr_to_id[spkr_name]]).view(1).numpy()
        return spkr_id


    def _get_noiselabel(self, idx):
        spkr_name = parse_speaker(self.audio_files[idx], self.multispkr)
        if spkr_name[:3]=="Ses":
            return torch.LongTensor([1]).view(1).numpy() # noisy
        else:
            return torch.LongTensor([0]).view(1).numpy() # clean
...
class F0Dataset(torch.utils.data.Dataset):
    def __init__(self, training_files, segment_size, sampling_rate,
        ...
        self.audio_files, _,_ = training_files
        random.seed(1234)
        self.segment_size = segment_size
        self.sampling_rate = sampling_rate
        self.split = split
        ...
        if self.multispkr:
            ...
            self.id_to_spkr = spkrs
            self.spkr_to_id = {k: v for v, k in enumerate(self.id_to_spkr)}
            print("self.spkr_to_id in F0",self.spkr_to_id)
    ...
    def __getitem__(self, index):
        ...
        #print("##########in dataset, feats{}, file {}".format(len(feats), str(filename)))
        #print("##########in dataset, feats",feats)

...
class CodeGenerator(Generator):
    def __init__(self, h):
        ...
        #if h.get('model_noise', None):
        #self.noise_rep = nn.Embedding(2, 128)
        
        self.f0 = h.get('f0', None)
        self.multispkr = h.get('multispkr', None)

        if self.multispkr:
            self.spkr = nn.Embedding(1, h.embedding_dim) # 233 for lrs2
        ...
        #########################################################################
        #################### put false if no f0 #################################
        if False: #h.get('f0_quantizer_path', None):
            assert self.f0, "Requires F0 set"
    ...
    def _upsample(signal, max_frames):
        #breakpoint()
        #print("signal.dim(), max_frames",signal.dim(), max_frames)
        ...
        submaxframes = signal.shape[2] * signal.shape[3]
        signal = signal.view(bsz, channels, submaxframes)
    ...
    def forward(self, **kwargs):
        ...
        code_metrics = None
        # print("code in model shape",kwargs['code'].shape)
        if self.code_vq and kwargs['code'].dtype is torch.int64:
            ...
            #print("in model 0 x:",x.shape)

        f0_commit_losses = None
        f0_metrics = None
        #print("********9n model f0,code",kwargs['f0'].shape,kwargs['code'].shape)
        if self.vq:
            ...
            # print("before",len(zs[0][0]))
            # if 'pitch' in kwargs:
            #     zs = [kwargs['pitch']]
            # print("after",len(zs[0]))
        ...
        ##################### put false if no f0 ###########
        if False: #self.f0:
            #print("x shape, f0shape",x.shape[-1],kwargs['f0'].shape[-1])
            if x.shape[-1] < kwargs['f0'].shape[-1]:
                x = self._upsample(x, kwargs['f0'].shape[-1])
            else:
                #print("**********f0, x shape",kwargs['f0'].shape,x.shape)
        #
                kwargs['f0'] = self._upsample(kwargs['f0'], x.shape[-1])
        #
        #         if self.quantizer:
        #             zs = self._upsample(zs[0].unsqueeze(0),x.shape[-1])
        #         print("kwargs",kwargs['f0'].shape,x.shape,zs.shape)

            ###########################################################
            ######################reducing x size based on f0##########
            x = x[:,:,:kwargs['f0'].shape[-1]]

            x = torch.cat([x, kwargs['f0'][:,:,:x.shape[-1]]], dim=1)
        ...
        if False: #True:
            noise_rep = self.noise_rep(kwargs['noise_label']).transpose(1, 2)
            noise_rep = self._upsample(noise_rep, x.shape[-1])
            x = torch.cat([x, noise_rep], dim=1)


        for k, feat in kwargs.items():
            if k in ['spkr', 'noise_label', 'code', 'f0','pitch']:
                continue
            print("feat", k)

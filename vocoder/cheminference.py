#python inference.py --checkpoint_file /ssd_scratch/cvit/neha/sr_checkpoints/ -n 10 --output_dir generations --input_code_file ./datasets/vctkiemo/hubert100/test.txt
...
def init_worker(queue, arguments):
    ...
    os.makedirs(os.path.join(a.output_dir,"f0"),exist_ok=True)
...
def inference(item_index):
    ...
    fname_out_name = '_'.join(filename.split('/')[-2:])[:-4]
    ...
    #
            ...
            # print("After code removal",code['code'].shape)
    ...
    #     del new_code['f0']
    #     new_code['f0'] = code['f0']
    #new_code['noise_label'] = torch.tensor([[0]]).to(device)
    #new_code['spkr'] = torch.tensor([[7]]).to(device)
...
# print("f0 shape",f0.shape)
    ...
    output_f0 = os.path.join(a.output_dir,"f0", fname_out_name + '_f0')
    # np.save(output_f0,f0.detach().cpu())
    audio = librosa.util.normalize(audio.astype(np.float32))
    write(output_file, h.sampling_rate, audio)


    print("multispkr:",h.get('multispkr', None),a.vc)
...
def main():
    ...
    parser.add_argument('--input_code_file', default='./datasets/LJSpeech/hubert100/trail.txt')

...
def load_audio_visual(manifest_path, max_keep, min_keep, frame_rate, label_paths, label_rates, tol=0.5):  # 0.1):
    def is_audio_label_aligned(audio_dur, label_durs):
        #print('%%%%%%%%%%%%%%',abs(audio_dur[0] - label_dur[0])<tol, audio_dur[0], label_dur[0], tol)
    ...
    #print('*********manifest_path**********',manifest_path, label_path)
    with open(manifest_path) as f:
        ...
        for ind, line in enumerate(f):
            ...
            if min_keep is not None and sz < min_keep:
                ...
                print('in ds, unaligned files',items[0], sz/frame_rate, dur_from_label_list[ind], sz)
                n_unaligned += 1
            else:
                video_path = items[1]
                audio_path = items[2]
                ...
                sizes.append(sz)
    tot = ind + 1
    print('@@@@@@@@@',f"max_keep={max_keep}, min_keep={min_keep}, ",f"loaded {len(names)}, skipped {n_short} short and {n_long} long and {n_unaligned} unaligned, ")
    logger.info(
        (
...
class AVHubertDataset(FairseqDataset):
    ...
    def get_label(self, index, label_idx):
        if self.store_labels:
            #print('self.store_labels')
            label = self.label_list[label_idx][index]
        else:
            #print(self.label_paths[label_idx])
    ...
    def __getitem__(self, index):
        ...
        #print('in dataset, labels:', labels)
        #print(len(labels[0]), audio_feats.shape, video_feats.shape)
        ...
        #for i, vf in enumerate(video_feats):
        #    img = vf.squeeze(-1).numpy()
        #    min_val = np.min(img)
        #    max_val = np.max(img)
        #    rescaled_img = ((img - min_val) / (max_val - min_val)) * 255
        #    cv2.imwrite('croppedvid/'+str(i)+'.jpg', rescaled_img)

        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': torch.zeros_like(audio_feats), "label_list": labels}

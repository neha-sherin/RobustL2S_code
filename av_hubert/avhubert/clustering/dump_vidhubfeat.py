...
class HubertFeatureReader(object):
    def __init__(self, ckpt_path, layer, max_chunk=1600000, custom_utils=None):
        ...
        print(model)
        #breakpoint()
        self.model = model[0].encoder.w2v_model.eval().cuda()
        #self.model = model[0].eval().cuda()
        self.task = task
        self.task.modalities = ['video']
        self.layer = layer
    ...
    def get_feats(self, path, ref_len=None):
        ...
        with torch.no_grad():
            ...
            #source = {'audio': torch.zeros_like(audio_feats), 'video': video_feats}
...
def dump_feature(
    ...
    featfilepath = os.path.join(feat_dir, 'vid_feature_files')
    os.makedirs(featfilepath, exist_ok=True)

    feat_f = NpyAppendArray(feat_path)
    with open(leng_path, "w") as leng_f:
        for path, nsample in tqdm.tqdm(iterator, total=num):
            feat = reader.get_feats(path, nsample)
            folderpath = os.path.join(featfilepath, '/'.join(path[-1].split(':')[-1].split('/')[:-1] ) )
            os.makedirs(folderpath, exist_ok=True)
            np.save(os.path.join(featfilepath, path[-1].split(':')[-1] ), feat.cpu().numpy())
